...
router.post('/login', (req, res, next) => {
    pool.getConnection((error, conn) => {
        ...
        conn.query(query, [req.body.email], (error, results, fields) => {
            ...
            if (error) { return res.status(500).send({ error: error }) }
            if (results.length < 1) {
                return res.status(401).send({ mensagem: "Falha na autenticaçao" })
            }
    ...
    pool.getConnection((err, conn) => {
        ...
        conn.query(
            ...
            SET nome_usuario = ?, 
                ...
                tarefas_id_tarefas = ? 
            WHERE id_usuario = ?`,
            [nome_usuario, data_de_nascimento, cep, tarefas_id_tarefas, id_usuario],
            (errQuery, result) => {
                conn.release();
                ...
                res.status(202).json({ mensagem: 'Usuário atualizado', id_usuario });
            }
        );
    });
});
...
router.delete('/:id_usuario', (req, res) => {
    ...
    pool.getConnection((err, conn) => {
        ...
        conn.query('DELETE FROM usuarios WHERE id_usuarios = ?', [id_usuario], (errQuery, result) => {
            conn.release();
            ...
            res.status(202).json({ mensagem: 'Usuário deletado', id_usuario });
        });
    });
});
